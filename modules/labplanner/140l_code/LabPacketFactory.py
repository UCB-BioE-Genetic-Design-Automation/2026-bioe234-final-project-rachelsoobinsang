...
class LabPacketFactory:
    ...
    def pcrSheets(self, expName, pcrSteps, inventory):
        ...
        pcrLabSheets = []
        num_samples = len(pcrSteps)
        calc_result = calculate_master_mix("PCR", num_samples)
        reactions = [(Reagent[r], v)
    ...
    def ligateSheets(self, expName, ligateSteps, inventory):
        ...
        num_samples = len(ligateSteps)
        calc_result = calculate_master_mix("Ligate", num_samples)
        reaction = [(Reagent[r], v)
                    for r, v in calc_result["Scaled Recipe"].items()]
        recipe = Recipe(None, reaction)
    ...
    def ggSheets(self, expName, ggSteps, inventory):
        ...
        for enzyme in enzyme_to_steps:
            num_samples = len(enzyme_to_steps[enzyme])
            calc_result = calculate_master_mix("Golden_Gate", num_samples)
            reaction = [(Reagent[r], v)
                        for r, v in calc_result["Scaled Recipe"].items()]
            reaction.append((enzyme, 1))
            recipe = Recipe(None, reaction)
    ...
    def gibsonSheets(self, expName, gibsonSteps, inventory):
        ...
        num_samples = len(gibsonSteps)
        calc_result = calculate_master_mix("Gibson", num_samples)
        reactionGib = [(Reagent[r], v)
                       for r, v in calc_result["Scaled Recipe"].items()]
